...
class TestServerArgsDllmValidation(unittest.TestCase):
    def test_fa4_accepts_dynamic_block_tiers(self):
        ...
        with tempfile.NamedTemporaryFile("w", suffix=".yaml") as f:
            ...
            # Decode CUDA graph enabled: exercises the attention-backend gate so
            # this actually verifies fa4 is accepted (not silently forced to
            # flashinfer). Without this, cuda_graph_config is None and the gate
            # is skipped, making the test pass vacuously.
        ...
        # fa4 must survive the dllm attention-backend resolution unchanged.
